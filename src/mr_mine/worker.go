...
func Worker(mapf func(string, string) []KeyValue,
	...
	for {
		...
		for reply.Task == MAP {
			...
			if os.Getenv("LOG") == "1" {
				fmt.Printf("intermediateFilePrefix %v\n", args.Payload.IntermediateFilePrefix)
			}
			WorkerCall(&args, &reply)
		}
...
func reduceTask(reducef func(string, []string) string, prefixList []string, reduceNumber int) {
	...
	fmt.Printf("Finished Reduce task for number: %d\n", reduceNumber)
}
...
func WorkerCall(args *WorkerArgs, reply *CoordReply) {
	...
	if os.Getenv("LOG") == "1" {
		fmt.Printf("Sending status %v to coord\n", args.Status)
	}

	if ok {
		fmt.Printf("Received %v task\n", reply.Task)
	} else {
