...
func TestConcurrentStarts2B(t *testing.T) { // Didn't pass
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
func TestRejoin2B(t *testing.T) { // May Fail
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
func TestFigure82C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		for i := 0; i < servers; i++ {
			if cfg.rafts[i] != nil {
				temp := rand.Int()
				_, _, ok := cfg.rafts[i].Start(temp)
				if ok {
					leader = i
					Debug(dInfo, "Server %v: Start(%v)\n", i, temp)
				}
			}
		}
